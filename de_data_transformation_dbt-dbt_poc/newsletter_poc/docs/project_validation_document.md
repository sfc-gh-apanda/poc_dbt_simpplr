...
```
  ...
  │  DBT_UDL.NEWSLETTER_HIST         ← UPDATE deactivate + INSERT new active       │
  │    (master SCD-2 accumulator, active_flag tracked)                          │
  │  DBT_UDL.NEWSLETTER              ← TRUNCATE + INSERT WHERE active_flag=TRUE    │
  │    (derived current state — Time Travel preserved)                          │
  │  DBT_UDL.NEWSLETTER_INTERACTION  ← MERGE delta from WRK (Time Travel preserved)│
  │  DBT_UDL.NEWSLETTER_CATEGORY     ← MERGE delta from WRK (Time Travel preserved)│
  │                                                                             │
...
| `DBT_UDL` | Published target tables | NEWSLETTER (derived from HIST), NEWSLETTER_INTERACTION, NEWSLETTER_CATEGORY, NEWSLETTER_HIST, pipeline_complete view |
| `DBT_UDL_BATCH_PROCESS` | Staging, work, seeds, procedures, reprocess queue | 4 staging tables, 3 wrk tables, 6 seed tables, stored procedures, REPROCESS_REQUEST |
| `DBT_UDL` | dbt delta work tables | 3 wrk_* tables (delta-only), 1 sentinel view |
| `DBT_UDL_BATCH_PROCESS` | Staging, work, seeds, stored procedures | stg_*, wrk_*, ref_*, 8 procedures (merge helper, publish, archive, retry, artifact logging), REPROCESS_REQUEST |
...
| `DBT_UDL_BATCH_PROCESS.SHARED_STG_*` | `DBT_UDL_BATCH_PROCESS.STG_NEWSLETTER`, `STG_NEWSLETTER_INTERACTION`, etc. | Staging layer (parsing + hashing) |
| `DBT_UDL_BATCH_PROCESS.WRK_*` | `DBT_UDL_BATCH_PROCESS.WRK_NEWSLETTER`, `WRK_NEWSLETTER_INTERACTION`, `WRK_NEWSLETTER_CATEGORY` | Work/mart tables (dedup + enrich) |
| `DBT_UDL.NEWSLETTER` | `DBT_UDL.NEWSLETTER` (derived from NEWSLETTER_HIST WHERE active_flag=TRUE) | User-facing current state (Time Travel preserved via TRUNCATE+INSERT) |
| `DBT_UDL.NEWSLETTER_INTERACTION` | `DBT_UDL.NEWSLETTER_INTERACTION` (via MERGE from wrk delta) | User-facing published table (Time Travel preserved) |
| `DBT_UDL.NEWSLETTER_CATEGORY` | `DBT_UDL.NEWSLETTER_CATEGORY` (via MERGE from wrk delta) | User-facing published table (Time Travel preserved) |
| `DBT_UDL.NEWSLETTER_HIST` | `DBT_UDL.NEWSLETTER_HIST` (master SCD-2 accumulator) | All versions with active_flag tracking — single source of truth |
...
| Hash-based deduplication | `LEFT JOIN DBT_UDL published table ON hash_value` | Same MD5 hash logic over business columns |
...
| Publish to DBT_UDL (DELETE+INSERT) | HIST-as-master publish | NEWSLETTER: deactivate HIST → insert active → TRUNCATE+INSERT DBT_UDL.NEWSLETTER. INTERACTION/CATEGORY: delta MERGE |
| Archive raw data | `PRC_DBT_ARCHIVE_RAW_DATA` procedure | INSERT into archive + DELETE from raw |
| SCD-2 history (NEWSLETTER_HIST) | `DBT_UDL.NEWSLETTER_HIST` (master accumulator) | active_flag-based SCD-2 — same pattern as Scala pipeline |
...
2. Hash-based dedup: new records are compared against published DBT_UDL table `hash_value` — unchanged records are skipped
...
- `DBT_UDL.NEWSLETTER` is derived: `TRUNCATE TABLE; INSERT INTO ... SELECT * FROM NEWSLETTER_HIST WHERE ACTIVE_FLAG = TRUE`
...
**What:** All user-facing DBT_UDL tables + NEWSLETTER_HIST are updated atomically in a single transaction.
...
3. **NEWSLETTER (HIST-as-master):**
   ...
   - `TRUNCATE TABLE DBT_UDL.NEWSLETTER; INSERT INTO ... SELECT ... WHERE ACTIVE_FLAG=TRUE`
...
- **Preserves Snowflake Time Travel** on DBT_UDL.NEWSLETTER — TRUNCATE+INSERT is DML, not DDL
...
**Full-load mode:** When `is_full_load: true` (or `entity_specific_full_load` targets the entity):
- **Staging:** `UNION ALL` raw + archive tables to reconstruct the complete dataset
- **WRK models:** Skip hash-based deduplication (all records pass through); ID column resets to `ROW_NUMBER()` starting from 1
- **Publish procedure:** `DELETE FROM NEWSLETTER_HIST WHERE id != -1` (purge + reload); `TRUNCATE + INSERT` for INTERACTION/CATEGORY
- **Step-level logging:** Each entity's publish step is individually tracked (RUNNING → SUCCESS/ERROR/SKIPPED) in `DBT_MODEL_LOG`
- **Dynamic column discovery:** Column lists built from `INFORMATION_SCHEMA.COLUMNS` (no hardcoded column names in procedure)
...
|-------|--------------|------------|--------------|
| `wrk_newsletter` | `DBT_UDL.NEWSLETTER` | 2 seed lookups (status, recipient type). Tenant-aware ID prefixing. | `NEWSLETTER_HIST` → `DBT_UDL.NEWSLETTER` |
| `wrk_newsletter_interaction` | `DBT_UDL.NEWSLETTER_INTERACTION` | 5 seed lookups. Device type classification from user agent. | `DBT_UDL.NEWSLETTER_INTERACTION` (MERGE) |
| `wrk_newsletter_category` | `DBT_UDL.NEWSLETTER_CATEGORY` | Data source derivation. Null-safe defaults. | `DBT_UDL.NEWSLETTER_CATEGORY` (MERGE) |
...
Additionally, all DBT_UDL target tables carry `published_by_run_id` (the dbt invocation that triggered publish) and `published_at`, providing a unified publish-event identifier across all three entities.
...
|----------|---------|---------|
| `is_full_load` | `false` | When `true`: staging UNIONs raw+archive; wrk skips hash dedup & resets IDs; procedure TRUNCATEs targets |
| `entity_specific_full_load` | `'none'` | Comma-separated entity names (e.g., `'NEWSLETTER,NEWSLETTER_INTERACTION'`) for per-entity full load |
...
| `enable_publish` | `true` | Toggle HIST-as-master publish to DBT_UDL |
...
| Step | Script | Creates |
|------|--------|---------|
| 1 | `account_bootstrap.sql` | Database, schemas (DBT_UDL, DBT_UDL_BATCH_PROCESS, DBT_EXECUTION_RUN_STATS, SHARED_SERVICES_STAGING), source/archive tables, 4 DBT_UDL target tables, REPROCESS_REQUEST + _HIST, sample data, access integration |
| 2 | `audit_setup.sql` | `DBT_RUN_LOG`, `DBT_MODEL_LOG` (batch_id VARCHAR(150)), 4 summary views |
...
CALL DBT_UDL_BATCH_PROCESS.PRC_DBT_FULL_RUN_WITH_RETRY(
    '@my_git_stage', '/newsletter_poc', 'dev', 2, NULL
);

-- Build with artifact logging (captures test + model results)
CALL DBT_UDL_BATCH_PROCESS.PRC_DBT_EXECUTE_AND_LOG_BUILD(
...
| 5 | Publish Verification | NEWSLETTER_HIST accumulates versions, DBT_UDL.NEWSLETTER = active HIST, INTERACTION/CATEGORY via MERGE |
...
| Deduplication | Custom hash comparison | Same MD5 hash, dedup against published DBT_UDL |
...
| Publish to DBT_UDL | DELETE + INSERT (swap) | HIST-as-master (deactivate+insert HIST, TRUNCATE+INSERT newsletter, MERGE others) |
| Time Travel on DBT_UDL | Lost on each publish cycle | Preserved on all tables (TRUNCATE+INSERT and MERGE are DML, not DDL) |
...
- [ ] Unchanged records are skipped (hash-based dedup against DBT_UDL published tables)
...
- [ ] DBT_UDL.NEWSLETTER matches NEWSLETTER_HIST WHERE ACTIVE_FLAG=TRUE

### Publish & Archive
- [ ] DBT_UDL.NEWSLETTER derived correctly from NEWSLETTER_HIST active records
- [ ] INTERACTION and CATEGORY updated via MERGE (new rows inserted, changed rows updated)
- [ ] Time Travel works on DBT_UDL tables after publish (`SELECT * FROM DBT_UDL.NEWSLETTER AT(OFFSET => -60)`)
...
- [ ] published_by_run_id present on all DBT_UDL target tables (NEWSLETTER_HIST, INTERACTION, CATEGORY)
