...
--   ROUND 5:  Publish Verification (HIST-as-master + MERGE → DBT_UDL)
...
--   ROUND 8:  Full Load Mode (is_full_load=true → skip dedup, reset IDs, TRUNCATE targets)
--   ROUND 9:  Entity-Specific Full Load (entity_specific_full_load='NEWSLETTER')
--   ROUND 10: Step-Level Logging Verification (per-entity publish audit)
--
-- How to use:
...
FROM DBT_UDL_BATCH_PROCESS.WRK_NEWSLETTER
UNION ALL
SELECT 'WRK_NEWSLETTER_INTERACTION', COUNT(*)
FROM DBT_UDL_BATCH_PROCESS.WRK_NEWSLETTER_INTERACTION
UNION ALL
SELECT 'WRK_NEWSLETTER_CATEGORY', COUNT(*)
FROM DBT_UDL_BATCH_PROCESS.WRK_NEWSLETTER_CATEGORY;
-- EXPECTED:
...
-- 1H. Publish verification — DBT_UDL tables
SELECT 'DBT_UDL.NEWSLETTER' AS table_name, COUNT(*) AS row_count FROM DBT_UDL.NEWSLETTER
UNION ALL
SELECT 'DBT_UDL.NEWSLETTER_INTERACTION', COUNT(*) FROM DBT_UDL.NEWSLETTER_INTERACTION
UNION ALL
SELECT 'DBT_UDL.NEWSLETTER_CATEGORY', COUNT(*) FROM DBT_UDL.NEWSLETTER_CATEGORY
UNION ALL
SELECT 'DBT_UDL.NEWSLETTER_HIST', COUNT(*) FROM DBT_UDL.NEWSLETTER_HIST;
-- EXPECTED: NEWSLETTER=4, INTERACTION=4, CATEGORY=3, HIST=4

-- 1I. Seed tables loaded
SELECT 'ref_newsletter_status' AS seed, COUNT(*) AS rows FROM DBT_UDL_BATCH_PROCESS.REF_NEWSLETTER_STATUS
UNION ALL SELECT 'ref_newsletter_interaction_type', COUNT(*) FROM DBT_UDL_BATCH_PROCESS.REF_NEWSLETTER_INTERACTION_TYPE
UNION ALL SELECT 'ref_newsletter_delivery_system_type', COUNT(*) FROM DBT_UDL_BATCH_PROCESS.REF_NEWSLETTER_DELIVERY_SYSTEM_TYPE
UNION ALL SELECT 'ref_newsletter_recipient_type', COUNT(*) FROM DBT_UDL_BATCH_PROCESS.REF_NEWSLETTER_RECIPIENT_TYPE
UNION ALL SELECT 'ref_newsletter_click_type', COUNT(*) FROM DBT_UDL_BATCH_PROCESS.REF_NEWSLETTER_CLICK_TYPE
UNION ALL SELECT 'ref_newsletter_block_type', COUNT(*) FROM DBT_UDL_BATCH_PROCESS.REF_NEWSLETTER_BLOCK_TYPE;
...
SELECT COUNT(*) AS wrk_newsletter_count FROM DBT_UDL_BATCH_PROCESS.WRK_NEWSLETTER;
-- EXPECTED: 4
...
SELECT COUNT(*) AS hist_count FROM DBT_UDL.NEWSLETTER_HIST;
...
-- 2I. DBT_UDL.NEWSLETTER should have 5 total (4 existing + 1 new, 1 updated in-place)
SELECT COUNT(*) AS udl_newsletter_count FROM DBT_UDL.NEWSLETTER;
-- EXPECTED: 5
...
-- 2L. DBT_UDL.NEWSLETTER should have exactly the active HIST records
SELECT
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER) AS udl_count,
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER_HIST WHERE ACTIVE_FLAG = TRUE) AS hist_active_count,
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER) = (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER_HIST WHERE ACTIVE_FLAG = TRUE) AS counts_match;
...
-- 3C. DBT_UDL.NEWSLETTER should still have 5 records (delete just updates the row)
SELECT COUNT(*) AS udl_newsletter_count FROM DBT_UDL.NEWSLETTER;
-- EXPECTED: 5
...
-- 4A. DBT_UDL.NEWSLETTER state before no-op run (wrk is delta-only, check published table)
SELECT code, name, hash_value
FROM DBT_UDL.NEWSLETTER
WHERE code = '85db08a2-3579-49b8-b4a4-1d80fd9021a7';
-- NOTE: Save hash_value — DBT_UDL.NEWSLETTER should NOT change after next build (no delta)
...
SELECT COUNT(*) AS wrk_newsletter_count FROM DBT_UDL_BATCH_PROCESS.WRK_NEWSLETTER;
-- EXPECTED: 0 (hash dedup against DBT_UDL.NEWSLETTER filters out all unchanged records)
...
-- ║  Pattern: wrk delta → NEWSLETTER_HIST → DBT_UDL.NEWSLETTER + MERGE others  ║
-- ╚═══════════════════════════════════════════════════════════════════════════╝
...
-- 5A. DBT_UDL.NEWSLETTER = active NEWSLETTER_HIST records
SELECT
    'NEWSLETTER'             AS entity,
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER_HIST WHERE ACTIVE_FLAG = TRUE) AS hist_active_count,
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER)                               AS udl_count,
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER_HIST WHERE ACTIVE_FLAG = TRUE) =
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER)                               AS match
UNION ALL
SELECT
    'NEWSLETTER_INTERACTION',
    NULL,
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER_INTERACTION),
    TRUE
UNION ALL
SELECT
    'NEWSLETTER_CATEGORY',
    NULL,
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER_CATEGORY),
    TRUE;
-- EXPECTED: NEWSLETTER MATCH = TRUE, all entities have expected row counts

-- 5B. Data content parity: spot-check a record in DBT_UDL.NEWSLETTER vs NEWSLETTER_HIST
SELECT 'NEWSLETTER_HIST' AS source_table, code, name, subject, hash_value, active_flag
FROM DBT_UDL.NEWSLETTER_HIST WHERE code = '202a20ae-ba80-4d69-ad0f-46febe2e293c' AND ACTIVE_FLAG = TRUE
UNION ALL
SELECT 'NEWSLETTER', code, name, subject, hash_value, TRUE
FROM DBT_UDL.NEWSLETTER WHERE code = '202a20ae-ba80-4d69-ad0f-46febe2e293c';
-- EXPECTED: Both rows identical (DBT_UDL.NEWSLETTER derived from active HIST)
...
SELECT COUNT(*) AS stg_newsletter_count FROM DBT_UDL_BATCH_PROCESS.STG_NEWSLETTER;
...
SELECT COUNT(*) AS pending_requests FROM DBT_UDL_BATCH_PROCESS.REPROCESS_REQUEST WHERE STATUS = 'PENDING';
-- EXPECTED: 0

-- 7D. Capture current DBT_UDL state before reprocessing
SELECT code, name, hash_value FROM DBT_UDL.NEWSLETTER
WHERE code = '85db08a2-3579-49b8-b4a4-1d80fd9021a7';
...
SELECT COUNT(*) AS interaction_count FROM DBT_UDL.NEWSLETTER_INTERACTION
...
INSERT INTO DBT_UDL_BATCH_PROCESS.REPROCESS_REQUEST
...
SELECT * FROM DBT_UDL_BATCH_PROCESS.REPROCESS_REQUEST ORDER BY REQUEST_ID;
...
SELECT COUNT(*) AS stg_interaction_count FROM DBT_UDL_BATCH_PROCESS.STG_NEWSLETTER_INTERACTION
...
SELECT COUNT(*) AS remaining_pending FROM DBT_UDL_BATCH_PROCESS.REPROCESS_REQUEST WHERE STATUS = 'PENDING';
...
FROM DBT_UDL_BATCH_PROCESS.REPROCESS_REQUEST_HIST
ORDER BY REQUEST_ID;
-- EXPECTED: 3 rows — all STATUS='COMPLETED', BATCH_RUN_ID=1007,
--           PROCESSED_BY_RUN_ID = dbt invocation_id of the run

-- 7M. DBT_UDL tables should still be consistent
SELECT
    'NEWSLETTER'             AS entity,
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER_HIST WHERE ACTIVE_FLAG = TRUE) AS hist_active,
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER)                               AS udl_count,
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER_HIST WHERE ACTIVE_FLAG = TRUE) =
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER)                               AS match;
-- EXPECTED: match = TRUE
...
SELECT COUNT(*) AS remaining FROM DBT_UDL_BATCH_PROCESS.REPROCESS_REQUEST WHERE STATUS = 'PENDING';
-- EXPECTED: 0

SELECT COUNT(*) AS hist_count FROM DBT_UDL_BATCH_PROCESS.REPROCESS_REQUEST_HIST
WHERE RECORD_CODE = 'new-nl-round2-uuid-1234567890ab';
-- EXPECTED: 1 (moved to history as COMPLETED)


-- ╔═══════════════════════════════════════════════════════════════════════════╗
-- ║  ROUND 8: FULL LOAD MODE (is_full_load=true)                           ║
-- ║  Pattern: Skip hash dedup, reset IDs to 1, TRUNCATE HIST,             ║
-- ║           rebuild target tables from scratch                            ║
-- ║  Validates: wrk models (skip dedup + ID reset) + procedure (full load) ║
-- ╚═══════════════════════════════════════════════════════════════════════════╝

-- ─────────────────────────────────────────────────────────────────────────────
-- BEFORE: Capture current state before full load rebuild
-- ─────────────────────────────────────────────────────────────────────────────

-- 8A. Current NEWSLETTER_HIST state (should have multiple versions from prior rounds)
SELECT
    COUNT(*)                              AS hist_total,
    SUM(IFF(active_flag, 1, 0))           AS active_versions,
    SUM(IFF(NOT active_flag, 1, 0))       AS inactive_versions,
    MAX(id)                               AS max_id
FROM DBT_UDL.NEWSLETTER_HIST;
-- EXPECTED: hist_total > 5, some inactive versions from SCD-2, max_id > 5

-- 8B. Current DBT_UDL.NEWSLETTER_INTERACTION state
SELECT COUNT(*) AS interaction_count, MAX(id) AS max_id
FROM DBT_UDL.NEWSLETTER_INTERACTION;
-- NOTE: Save max_id — should reset to lower after full load

-- 8C. Current DBT_UDL.NEWSLETTER_CATEGORY state
SELECT COUNT(*) AS category_count, MAX(id) AS max_id
FROM DBT_UDL.NEWSLETTER_CATEGORY;
-- NOTE: Save max_id — should reset to lower after full load


-- ─────────────────────────────────────────────────────────────────────────────
-- ACTION: Run dbt build with is_full_load=true (batch_run_id=1009)
--   When is_full_load=true:
--     wrk models: skip hash dedup (all source records pass through)
--     wrk models: ID resets to ROW_NUMBER() starting from 1
--     procedure:  NEWSLETTER_HIST: DELETE WHERE id != -1, then INSERT
--     procedure:  INTERACTION/CATEGORY: TRUNCATE then INSERT
-- ─────────────────────────────────────────────────────────────────────────────
-- EXECUTE DBT PROJECT
--     FROM @COMMON_TENANT_DEV.DBT_UDL.DBT_STAGE
--     PROJECT_ROOT = '/newsletter_poc'
...
-- ─────────────────────────────────────────────────────────────────────────────
-- AFTER: Validate full load results
-- ─────────────────────────────────────────────────────────────────────────────

-- 8D. WRK tables should have ALL source records (no hash dedup filtering)
SELECT 'WRK_NEWSLETTER' AS table_name, COUNT(*) AS row_count
FROM DBT_UDL_BATCH_PROCESS.WRK_NEWSLETTER
UNION ALL
SELECT 'WRK_NEWSLETTER_INTERACTION', COUNT(*)
...
SELECT 'WRK_NEWSLETTER_CATEGORY', COUNT(*)
FROM DBT_UDL_BATCH_PROCESS.WRK_NEWSLETTER_CATEGORY;
-- EXPECTED: All source records present (no records filtered by hash dedup)
--   Count should be >= the delta counts from prior rounds

-- 8E. IDs should start from 1 (not continue from MAX(id))
SELECT MIN(id) AS min_id, MAX(id) AS max_id, COUNT(*) AS total
FROM DBT_UDL_BATCH_PROCESS.WRK_NEWSLETTER;
-- EXPECTED: min_id=1, max_id=COUNT(*) (contiguous from 1)
...
-- 8F. NEWSLETTER_HIST: should contain ONLY the current batch (old versions purged)
SELECT
    COUNT(*)                              AS hist_total,
    SUM(IFF(active_flag, 1, 0))           AS active_versions,
    SUM(IFF(NOT active_flag, 1, 0))       AS inactive_versions,
...
FROM DBT_UDL.NEWSLETTER_HIST;
-- EXPECTED:
--   hist_total = WRK_NEWSLETTER count (all old versions deleted)
--   active_versions = hist_total (all records are fresh active versions)
--   inactive_versions = 0 (no old inactive versions remain)
--   min_id = 1, max_id = hist_total (IDs reset)

-- 8G. DBT_UDL.NEWSLETTER = active HIST (should still match)
SELECT
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER) AS udl_count,
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER_HIST WHERE ACTIVE_FLAG = TRUE) AS hist_active,
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER) =
    (SELECT COUNT(*) FROM DBT_UDL.NEWSLETTER_HIST WHERE ACTIVE_FLAG = TRUE) AS counts_match;
-- EXPECTED: counts_match = TRUE

-- 8H. INTERACTION + CATEGORY: should have been truncated and re-inserted
SELECT MIN(id) AS min_id, MAX(id) AS max_id, COUNT(*) AS total
FROM DBT_UDL.NEWSLETTER_INTERACTION;
-- EXPECTED: min_id=1 (IDs reset via full load WRK)

SELECT MIN(id) AS min_id, MAX(id) AS max_id, COUNT(*) AS total
FROM DBT_UDL.NEWSLETTER_CATEGORY;
-- EXPECTED: min_id=1 (IDs reset via full load WRK)

-- 8I. Step-level logging: each entity should have a SUCCESS entry
SELECT model_name, status, materialization, rows_affected, started_at, ended_at
FROM DBT_EXECUTION_RUN_STATS.DBT_MODEL_LOG
WHERE run_id IN (SELECT run_id FROM DBT_EXECUTION_RUN_STATS.DBT_RUN_LOG
                 ORDER BY run_started_at DESC LIMIT 1)
  AND model_name LIKE 'publish_%'
ORDER BY model_name;
-- EXPECTED: 3 rows:
--   publish_NEWSLETTER              → SUCCESS, materialization='full_load_hist'
--   publish_NEWSLETTER_CATEGORY     → SUCCESS, materialization='full_load'
--   publish_NEWSLETTER_INTERACTION  → SUCCESS, materialization='full_load'


-- ╔═══════════════════════════════════════════════════════════════════════════╗
-- ║  ROUND 9: ENTITY-SPECIFIC FULL LOAD                                    ║
-- ║  Pattern: Only NEWSLETTER is full-loaded; INTERACTION and CATEGORY     ║
-- ║           use the normal delta MERGE path                               ║
-- ╚═══════════════════════════════════════════════════════════════════════════╝
...
-- 9A. Current max IDs
SELECT 'NEWSLETTER_HIST' AS tbl, MAX(id) AS max_id FROM DBT_UDL.NEWSLETTER_HIST
UNION ALL
SELECT 'NEWSLETTER_INTERACTION', MAX(id) FROM DBT_UDL.NEWSLETTER_INTERACTION
UNION ALL
SELECT 'NEWSLETTER_CATEGORY', MAX(id) FROM DBT_UDL.NEWSLETTER_CATEGORY;
-- NOTE: NEWSLETTER IDs should reset, INTERACTION/CATEGORY should continue from MAX


-- ─────────────────────────────────────────────────────────────────────────────
-- ACTION: Run dbt build with entity_specific_full_load='NEWSLETTER'
--   Only NEWSLETTER is full-loaded; INTERACTION and CATEGORY remain delta
-- ─────────────────────────────────────────────────────────────────────────────
-- EXECUTE DBT PROJECT
--     FROM @COMMON_TENANT_DEV.DBT_UDL.DBT_STAGE
--     PROJECT_ROOT = '/newsletter_poc'
...
--         "batch_run_id": 1010,
--         "data_process_end_time": "2026-03-30 00:00:00",
--         "entity_specific_full_load": "NEWSLETTER"
--     }''';


-- ─────────────────────────────────────────────────────────────────────────────
-- AFTER: Validate entity-specific full load
-- ─────────────────────────────────────────────────────────────────────────────

-- 9B. WRK_NEWSLETTER: IDs should start from 1 (full load for this entity)
SELECT MIN(id) AS min_id, MAX(id) AS max_id
FROM DBT_UDL_BATCH_PROCESS.WRK_NEWSLETTER;
-- EXPECTED: min_id=1 (full load resets IDs)

-- 9C. WRK_NEWSLETTER_INTERACTION: IDs should continue from prior MAX
--     (entity_specific_full_load='NEWSLETTER' does NOT affect INTERACTION)
SELECT MIN(id) AS min_id, MAX(id) AS max_id
FROM DBT_UDL_BATCH_PROCESS.WRK_NEWSLETTER_INTERACTION;
-- EXPECTED: min_id > 1 (delta mode, IDs continue from existing MAX)
--   NOTE: If no delta (hash dedup filters all), WRK will be empty and this returns NULL

-- 9D. NEWSLETTER_HIST should be rebuilt (only current batch)
SELECT
    COUNT(*)                              AS hist_total,
    SUM(IFF(active_flag, 1, 0))           AS active_versions,
    SUM(IFF(NOT active_flag, 1, 0))       AS inactive_versions
FROM DBT_UDL.NEWSLETTER_HIST;
-- EXPECTED: inactive_versions=0, all records are fresh from this full load

-- 9E. INTERACTION/CATEGORY should have MERGED normally (not truncated)
--     Step-level logs should show 'merge' (not 'full_load') for INTERACTION/CATEGORY
SELECT model_name, status, materialization
FROM DBT_EXECUTION_RUN_STATS.DBT_MODEL_LOG
WHERE run_id IN (SELECT run_id FROM DBT_EXECUTION_RUN_STATS.DBT_RUN_LOG
                 ORDER BY run_started_at DESC LIMIT 1)
  AND model_name LIKE 'publish_%'
...
-- EXPECTED:
--   publish_NEWSLETTER              → SUCCESS, materialization='full_load_hist'
--   publish_NEWSLETTER_CATEGORY     → SUCCESS, materialization='merge'
--   publish_NEWSLETTER_INTERACTION  → SUCCESS, materialization='merge'


-- ╔═══════════════════════════════════════════════════════════════════════════╗
-- ║  ROUND 10: STEP-LEVEL LOGGING VERIFICATION                            ║
-- ║  Pattern: Verify that each entity's publish step is individually       ║
-- ║           logged with RUNNING → SUCCESS/ERROR/SKIPPED transitions       ║
-- ╚═══════════════════════════════════════════════════════════════════════════╝

-- 10A. All publish step logs from the most recent run
SELECT
    log_id,
    model_name,
    status,
    ...
    ended_at,
    error_message,
    DATEDIFF('second', started_at, ended_at) AS duration_sec
FROM DBT_EXECUTION_RUN_STATS.DBT_MODEL_LOG
WHERE model_name LIKE 'publish_%'
ORDER BY started_at DESC
LIMIT 10;
-- EXPECTED: Per-entity step logs with SUCCESS status, non-null ended_at,
--   rows_affected reflecting actual row counts, no error_message

-- 10B. Historical publish log analysis (across all runs)
SELECT
    run_id,
    model_name,
    status,
...
WHERE model_name LIKE 'publish_%'
ORDER BY started_at;
-- EXPECTED: One row per entity per run, showing the evolution of publish strategies
--   (hist_as_master / merge for delta runs, full_load_hist / full_load for full loads)
...
-- │ HIST-as-master publish            │ Round 5: DBT_UDL.NEWSLETTER = active HIST       │
...
-- │ Full load: skip hash dedup        │ Round 8: All records pass through wrk       │
-- │ Full load: ID reset to 1          │ Round 8: IDs start from 1, not MAX+1       │
-- │ Full load: HIST purge + rebuild   │ Round 8: DELETE WHERE id!=−1, then INSERT  │
-- │ Full load: TRUNCATE+INSERT target │ Round 8: INTERACTION/CATEGORY truncated    │
-- │ Entity-specific full load         │ Round 9: Only NEWSLETTER full-loaded       │
-- │ Entity-specific: mixed strategies │ Round 9: NL=full_load, others=merge        │
-- │ Step-level publish logging        │ Round 10: Per-entity RUNNING→SUCCESS       │
-- │ Dynamic column discovery          │ Round 8+: INFORMATION_SCHEMA-driven INSERTs│
