...
Writes to SIMPPLR_DBT_DEV.SIMPPLR_DBT_AUDIT.DBT_RUN_LOG.
...
{% macro log_run_start() %}
    {% if var('enable_audit_logging', true) %}
        {% set sql %}
            INSERT INTO {{ target.database }}.{{ var('audit_schema', 'SIMPPLR_DBT_AUDIT') }}.DBT_RUN_LOG (
                run_id,
                project_name,
                environment,
                run_started_at,
            ...
            SELECT
                ...
                CURRENT_ROLE()
            WHERE NOT EXISTS (
                SELECT 1 FROM {{ target.database }}.{{ var('audit_schema', 'SIMPPLR_DBT_AUDIT') }}.DBT_RUN_LOG
