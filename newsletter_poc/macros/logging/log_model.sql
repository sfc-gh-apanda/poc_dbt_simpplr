...
Writes to SIMPPLR_DBT_DEV.SIMPPLR_DBT_AUDIT.DBT_MODEL_LOG.
...
{% macro log_model_execution() %}
    {% if var('enable_audit_logging', true) and execute %}
        ...
        {% set sql %}
            INSERT INTO {{ target.database }}.{{ var('audit_schema', 'SIMPPLR_DBT_AUDIT') }}.DBT_MODEL_LOG (
                log_id,
                run_id,
                project_name,
                model_name,
            ...
            WHERE NOT EXISTS (
                SELECT 1 FROM {{ target.database }}.{{ var('audit_schema', 'SIMPPLR_DBT_AUDIT') }}.DBT_MODEL_LOG
                WHERE log_id = '{{ log_id }}'
            );
        {% endset %}
